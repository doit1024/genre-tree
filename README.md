...
Production: [https://www.doooit.me/demo/genre-tree](https://www.doooit.me/demo/genre-tree)

The app is mounted at **`/demo/genre-tree`** (`basePath` + `assetPrefix`). Keep `audioPath` in `data/tree.json` as a site-root path such as `/audio/twinkle-jazz.wav`; the player prefixes the base path.
...
Open [http://localhost:3000/demo/genre-tree](http://localhost:3000/demo/genre-tree) — not `/`. Seed demo: **小星星 / Twinkle Twinkle Little Star** — 1 root + 3 style children.
...
That builds with OpenNext and runs `wrangler` using `wrangler.jsonc`. The worker name is `genre-tree`. A route is declared for `www.doooit.me/demo/genre-tree*` on zone `doooit.me`.
...
2. Point each node’s `audioPath` in `data/tree.json` at that file (example: `"/audio/twinkle-jazz.mp3"`). Do **not** bake `/demo/genre-tree` into the JSON.
